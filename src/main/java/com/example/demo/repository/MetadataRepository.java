...
public interface MetadataRepository extends JpaRepository<Metadata, Long> {
    ...
    @Query(value = "SELECT sum(IF(MONTH = 'jan', total, 0)) AS 'jan', sum(IF(MONTH = 'feb', total, 0)) AS 'feb', sum(IF(MONTH = 'mar', total, 0)) AS 'mar',"
            ...
            +"FROM ( SELECT MIN(DATE_FORMAT(register_date, '%b')) AS MONTH, COUNT(name) AS total FROM metadata GROUP BY MONTH(register_date) ORDER BY MONTH(register_date) ) AS users",nativeQuery = true)
